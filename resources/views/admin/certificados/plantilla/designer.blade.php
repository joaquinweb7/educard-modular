...
<head>
    ...
    <title>Diseñador — {{ $plantilla->nombre }} · Certificados</title>
...
<div id="app">
    ...
    <div id="topbar">
        ...
        <a href="{{ route('admin.certificados.plantilla.index') }}" style="color:var(--text-muted);text-decoration:none;font-size:13px">Plantillas</a>
        <span class="sep">/</span>
        <span class="tpl-name" id="tpl-name-label">{{ $plantilla->nombre }}</span>
        <span class="dim-badge" id="dim-badge">{{ round(($plantilla->width ?? 1056) / 37.795276, 2) }} × {{ round(($plantilla->height ?? 816) / 37.795276, 2) }} cm</span>
        ...
        <form id="save-form" method="POST" action="{{ route('admin.certificados.plantilla.update', $plantilla) }}">
            @csrf @method('PUT')
            <input type="hidden" name="nombre" id="f-name" value="{{ $plantilla->nombre }}">
            <input type="hidden" name="design_json" id="f-design-json" value="{{ $plantilla->design_json ?? '{}' }}">
            <input type="hidden" name="width" id="f-width" value="{{ $plantilla->width ?? 1056 }}">
            <input type="hidden" name="height" id="f-height" value="{{ $plantilla->height ?? 816 }}">
        </form>
        ...
        <a href="{{ route('admin.certificados.plantilla.index') }}" class="tbtn tbtn-secondary"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> Cerrar</a>
    </div>
    ...
    <div id="workspace">
        ...
        <div id="toolbox">
            ...
            <button class="tool-btn" onclick="addElement('field','[NOMBRE_ESTUDIANTE]')">
                <span class="icon">👤</span> Nombre Estudiante
            </button>
            <button class="tool-btn" onclick="addElement('field','[CURSO]')">
                <span class="icon">🎓</span> Curso
            </button>
            <button class="tool-btn" onclick="addElement('field','[CODIGO]')">
                <span class="icon">#</span> Código
            </button>
            <button class="tool-btn" onclick="addElement('field','[CARNET]')">
                <span class="icon">🪪</span> Carnet
            </button>
            <button class="tool-btn" onclick="addElement('field','[EMAIL]')">
                <span class="icon">📧</span> Correo Electrónico
            </button>
            <button class="tool-btn" onclick="addElement('date','[FECHA_ACTUAL]')">
                <span class="icon">📆</span> Fecha de Emisión
            </button>
            ...
            <div class="var-grid">
                @foreach(['[NOMBRE_ESTUDIANTE]','[CURSO]','[CODIGO]','[CARNET]','[EMAIL]','[FECHA_ACTUAL]'] as $var)
        ...
        <div id="canvas-area" onclick="deselectAll($event)">
            <div id="card-canvas"
                 style="width:{{ $plantilla->width ?? 1056 }}px;height:{{ $plantilla->height ?? 816 }}px"
                 ondrop="handleDrop(event)" ondragover="event.preventDefault()">

                @if($plantilla->imagen)
                    <img id="canvas-bg" src="{{ asset('storage/'.$plantilla->imagen) }}" alt="Fondo">
                @else
                ...
                <div class="prop-group">
                    ...
                    <div class="prop-row">
                        ...
                        <select class="prop-input" id="p-font-family" onchange="applyProps()">
                            ...
                            <option value="sans-serif">Sans-Serif Normal</option>
                        </select>
                    </div>
                </div>
                <div class="prop-group">
...
<div id="settings-modal" style="
    ...
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:24px;width:360px">
        ...
        <div style="margin-bottom:12px">
            ...
            <input type="text" id="s-name" class="prop-input" style="width:100%" value="{{ $plantilla->nombre }}">
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
            <div>
                ...
                <input type="number" step="0.01" id="s-width" class="prop-input" style="width:100%" value="{{ round(($plantilla->width ?? 1056) / 37.795276, 2) }}">
            </div>
            <div>
                ...
                <input type="number" step="0.01" id="s-height" class="prop-input" style="width:100%" value="{{ round(($plantilla->height ?? 816) / 37.795276, 2) }}">
            </div>
        </div>
        <div style="margin-bottom:18px">
            ...
            <form id="bg-form" method="POST" action="{{ route('admin.certificados.plantilla.update', $plantilla) }}" enctype="multipart/form-data">
                @csrf @method('PUT')
                <input type="hidden" name="nombre" value="{{ $plantilla->nombre }}">
                <input type="file" name="imagen" id="s-bg" accept="image/png, image/jpeg" style="font-size:12px;color:var(--text)">
...
<script>
// ── State ────────────────────────────────────────────────────────────────────
const CANVAS_W = {{ $plantilla->width ?? 1056 }};
const CANVAS_H = {{ $plantilla->height ?? 816 }};
...
(function init() {
    // Load saved design
    const saved = @json($plantilla->design_json ?? '{}');
