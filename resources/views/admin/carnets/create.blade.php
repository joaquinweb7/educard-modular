...
@section('heading', 'Crear Nuevo Carnet')
@section('content')
...
<div class="panel">
    <div class="public-card">
        <h2 style="margin-bottom: 20px; font-size: 20px;">Registrar Carnet Manualmente</h2>
        ...
        <form action="{{ route('admin.carnets.store') }}" method="POST" novalidate>
            @csrf
            <div class="form-grid">
                {{-- Nombres --}}
                <div class="field">
                ...
                <div class="field">
                    ...
                    <input type="text" name="codigo_estudiante" value="{{ old('codigo_estudiante') }}" class="{{ $errors->has('codigo_estudiante') ? 'input-error' : '' }}" required>
                </div>

                {{-- Carrera --}}
                <div class="field">
                    <label>Carrera *</label>
                    <input type="text" name="carrera" value="{{ old('carrera') }}" class="{{ $errors->has('carrera') ? 'input-error' : '' }}" required>
                </div>

                {{-- Semestre --}}
                <div class="field">
                    <label>Semestre *</label>
                    <input type="text" name="semestre" value="{{ old('semestre') }}" class="{{ $errors->has('semestre') ? 'input-error' : '' }}" required>
                </div>
            ...
            <div class="actions" style="margin-top: 30px; display: flex; gap: 10px; justify-content: flex-end;">
                ...
                <button type="submit" class="btn primary">Guardar Carnet</button>
            </div>
        </form>
    </div>
</div>
