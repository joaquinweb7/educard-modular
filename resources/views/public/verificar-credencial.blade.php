...
<div class="public-wrap">
    <div class="public-card">
        ...
        <div style="text-align: center; margin-bottom: 24px;">
            ...
            <h2>Verificación de Credencial</h2>
            <p class="subtitle">Ingresa el código único del administrativo para validar su identidad</p>
        </div>
        ...
        <form method="GET">
            <div class="field">
                ...
                <div class="field-flex" style="display: flex; gap: 12px; align-items: stretch;">
                    <div style="flex: 1;">
                        <input type="text" name="code" placeholder="Ej. ADM-12345" value="{{ request('code') }}" required autocomplete="off">
                    </div>
                    ...
                    <div style="display: flex; gap: 24px; flex-wrap: wrap-reverse; align-items: center;">
                        <div style="flex: 1; min-width: 250px;">
                            ...
                            <tr>
                                ...
                                <td><span style="background:#dcfce7; color:#166534; font-weight:600; font-family:monospace; padding:4px 10px; border-radius:6px;">{{ $credencial->codigo_credencial }}</span></td>
                            </tr>
                            <tr>
                                <th>Nombres</th>
                            ...
                            <tr>
                                ...
                                <td>{{ $credencial->cargo_secundario }} @if($credencial->departamento) - {{ $credencial->departamento }} @endif</td>
                            </tr>
                            <tr>
                                <th>Validez</th>
                                <td>Del {{ \Carbon\Carbon::parse($credencial->fecha_emision)->format('d/m/Y') }} al {{ \Carbon\Carbon::parse($credencial->fecha_caducidad)->format('d/m/Y') }}</td>
                            </tr>
                            <tr>
                                <th>Estado</th>
                                <td>
                                    <span class="status-badge {{ strtolower($credencial->estado) == 'vigente' ? 'status-valid' : 'status-invalid' }}">
                                        <i data-lucide="{{ strtolower($credencial->estado) == 'vigente' ? 'check-circle' : 'alert-circle' }}" style="width:16px;height:16px;margin-right:6px;"></i>
                                        {{ strtoupper($credencial->estado) }}
                                    </span>
                                </td>
                            </tr>
                                </tbody>
                        ...
                        <div class="photo-container" style="flex: 0 0 150px; width: 150px; margin: 0 auto;">
                            @if($foto)
                                <img src="{{ Storage::url($foto) }}" alt="Foto Personal" style="width: 100%; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); aspect-ratio: 3/4; object-fit: cover;">
                            @else
            ...
            @else
                <div class="result-card" style="text-align: center; border-color: #fecaca; background: #fff5f5;">
                    <div class="empty-state" style="color:#991b1b; padding: 24px 0;">
                        ...
                        <h3 style="font-size: 18px; font-weight: 600; margin-bottom: 8px; color:#7f1d1d">Credencial no encontrada</h3>
                        <p style="font-size: 14px; margin:0">El código ingresado no coincide con ningún personal registrado en nuestro sistema.</p>
                    </div>
                </div>
            @endif
        @else
